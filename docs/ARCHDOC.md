...
### `archdoc export [--out <dir>] [--no-bodies] [--schema]`
...
Each document carries its front matter, the derived reverse relationships, its parsed sections, its links and its raw Markdown body. Bodies are never rendered: ArchDoc has no Markdown renderer and the dependency list is closed, so rendering and sanitising are the consumer's.

`sections` is keyed by anchor rather than title, because a heading may repeat and the anchor walk makes anchors unique within a document; keying by title would silently drop one. Every section is an H2, so no level is recorded, and a section includes its subsections. `required_sections` names what the type calls for, in order, as title and anchor pairs. Those are checked by L08 rather than guaranteed, so any of them may be absent from `sections`: a draft part-way through being written is the ordinary case, and the anchor is still given so a consumer can render a heading and say the section is missing.

Each link records the destination as written, the path it resolves to relative to the repository root, and the identifier or page name of the document there. That is what lets a consumer rewrite destinations into its own URL scheme without parsing Markdown. A destination carrying a URI scheme, or beginning with `/`, resolves to nothing and is reported as written.
...
With `--out`, writes a tree instead of a single document: `index.json` carrying every document without bodies, one file per document beside it at the document's own path, and `glossary.json` when there is a glossary. A page rendering one RFC should not have to fetch the whole specification, so `index.json` omits bodies whatever else was asked. With `--no-bodies`, omits document and section bodies everywhere, which is what an index page wants. With `--schema`, prints the JSON Schema the output conforms to and exits, and needs no repository.

Every array is an array, never null, and every object property the schema requires is present. The output is deterministic: nothing records when it was generated, so it can be cached, diffed and committed.
