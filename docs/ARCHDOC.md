...
3. For `accept` and `reject`: fails if there is no H1, or it does not read as the front matter gives it; fails if a required section is missing, or appears before one it must follow; fails if any required section is empty, other than `Open questions` and `Changelog`; fails if any unresolved `[[...]]` remains. For `accept` additionally: fails if the document has an `Open questions` section and it is non-empty. ADRs have no such section, so the check does not apply to them unless one is present as an extra section. `withdraw` performs no content checks.
...
### `archdoc export [--out <dir>] [--no-bodies] [--source] [--commit <rev>] [--schema]`

Writes the repository as JSON, so a consumer gets the documents and the relationships ArchDoc derives without reimplementing the front matter schema, the identifier rules or the reverse-relationship graph. `INDEX.md` already publishes that graph, but as Markdown links inside table cells; this is the same information in a shape a program can read.

Each document carries its front matter, the derived reverse relationships, its contents, its links, and its file where `--source` asked for it. Bodies are never rendered: ArchDoc has no Markdown renderer and the dependency list is closed, so rendering and sanitising are the consumer's.

The envelope carries `schema`, currently `2`; `config`, every setting `archdoc.json` holds; `has_glossary`; and `commit` where `--commit` gave one. The revision is recorded as written and is never read from git, because a revision taken from a working tree holding uncommitted edits would describe something other than the output.

`contents` is every heading in document order, each with its level, text, anchor, line and the prose beneath it up to the next heading of any level. No entry holds another's prose, so walking the list reads a body once, and a body is the lines as written, so a heading's line plus one is its first line. A body is absent where bodies were excluded and empty where one heading directly follows another, which is why it is absent rather than empty in the first case. The level is carried rather than implied by position, because heading levels need not descend one at a time. Anything before the first heading belongs to no entry and appears only in `source`.

A contents entry is not a section. `required_sections` names the H2 headings the type calls for, in order, as title and anchor pairs, and that is the unit L08 checks. Those are checked rather than guaranteed, so any of them may be absent from `contents`: a draft part-way through being written is the ordinary case, and the anchor is still given so a consumer can render a heading and say the section is missing.

Each link records the form that wrote it, the destination as written, the path it resolves to relative to the repository root, and the identifier or page name of the document there. The form is `inline` for `[text](destination)`, `image` for `![text](destination)` and `definition` for `[label]: destination`, whose text is the label, because an image destination becomes an image source and an inline one becomes an anchor. A reference use carries a label rather than a destination and is not listed: rewriting the definition carries its uses. That is what lets a consumer rewrite destinations into its own URL scheme without parsing Markdown. A destination carrying a URI scheme, or beginning with `/`, resolves to nothing and is reported as written.

The glossary is emitted a second time as structured terms, each with its definition and its recorded former names, so a consumer does not re-parse the page. The page itself still appears among the documents.

With `--out`, writes a tree instead of a single document: `index.json` carrying every document without bodies, one file per document beside it at the document's own path, and `glossary.json` when there is a glossary. A page rendering one RFC should not have to fetch the whole specification, so `index.json` omits bodies and source whatever else was asked, keeping the full outline. It also removes what an earlier export wrote there and this one does not, taking the previous run's files from the `index.json` already present and read before it is overwritten, so the command only ever removes a file an export wrote and a mistyped destination removes nothing. A path in that index is used only when it is local to the directory. Removals are reported on stderr. With `--no-bodies`, omits every contents body, which is what an index page wants. With `--source`, adds each document's file as it is on disk, front matter included, which is the only field that reproduces it. With `--schema`, prints the JSON Schema the output conforms to and exits, and needs no repository.

Every array is an array, never null, and every object property the schema requires is present. The output is deterministic: nothing records when it was generated, so it can be cached, diffed and committed. From `schema: 2` the shape only ever gains optional fields; version 2 replaced an anchor-keyed `sections` object that could not carry the order a document was written in and named H2 headings only, and it was the one break, taken while nothing consumed the output.

### `archdoc link [--suggest] [--apply]`

Resolves wiki links and, optionally, suggests new ones.

Default behaviour: for every document that is not frozen, replace each `[[X]]` with a markdown link:
...
Frozen documents containing `[[...]]` produce a warning naming the document, matching L16; they are never modified. The freeze is the push, so a document terminal in the working tree and not yet on `branch` is still editable and is resolved like any other, which is what makes a backfilled document's links resolvable: `new --backfill` creates an accepted document without passing the gate. Outside a git repository the question cannot be settled, so a terminal document is reported as an error and left alone. An unresolvable `[[X]]` in an editable document stays an error, because it is both the author's mistake and fixable. Findings from `link` follow `lint`'s exit codes; exit 1 stays reserved for usage errors.

With `--suggest`: additionally scan editable documents for bare identifiers and glossary terms not already inside a link or code span, and propose linking the first occurrence of each distinct target per document. Excludes the document's own identifier, every heading, front matter, code blocks, bare URLs, link reference definitions and the glossary page, which is skipped whole. It keys on the working tree status rather than on frozen: resolving a placeholder repairs something lint reports as an error, while offering a new link over prose already written is an addition and is not pressed on a document its author has called decided. A heading's text is a section's identity to L08, L09, L14 and the freeze gate, so neither `link` nor `link --suggest` rewrites one; an unresolved wiki link in a heading is reported by L16 and corrected by hand.
...
Nothing else. `internal/export` publishes a JSON Schema for its output and a test compares the schema against the structs by reflection rather than validating documents against it, because every JSON Schema validator would be another dependency. That catches a field added on one side and not the other, which is the drift that happens; it does not check that values match their declared types.

Markdown is handled with the standard library; the tool only needs headings, comments, fenced code blocks and links, and a full parser would be a liability when rewriting files in place.
