...
The generated workflow runs on push and pull request, downloads the `archdoc` binary at the version that ran `init` and verifies it against the published checksums, then runs `archdoc lint` and `archdoc index --check`. Its `working-directory` is the path from the git repository root to the directory holding `archdoc.json`, as git reports it, and is omitted when the two are the same.
...
The workflow's `ARCHDOC_VERSION` is raised to the version doing the update. It is kept as it is only when the running binary's version is not a published release.
